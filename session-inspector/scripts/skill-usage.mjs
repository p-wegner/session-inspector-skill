 ...
 *   node scripts/skill-usage.mjs --project webapp          # scope sessions AND the
 *       project-skill universe to one repo (matches folder / cwd / git-remote;
 *       `-`/`_`/separators are normalized, so `webapp` == `webapp`). "avail"
 *       then means "project sessions since the skill existed".
 *   node scripts/skill-usage.mjs --cwd                        # shorthand: --project <this cwd>
 *   node scripts/skill-usage.mjs --project webapp --repo-only   # report ONLY skills
 ...
 *   node scripts/skill-usage.mjs --project webapp --repo-only --cost   # + TOKEN TAX:
...
// hyphenated Claude session folder (`C--projects-acme-webapp`) and the
// underscored real repo path (`…\acme\webapp`) compare equal. Without
// this, `--project webapp` matches sessions but not the repo root, so
